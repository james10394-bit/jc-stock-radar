/* TWSE + TPEx Flow v2.4.13 — independent same-day closing-price snapshot. */
...
const latestClose = code => {
  const snapshot=market?.latest_quotes,quote=snapshot?.quotes?.[code];
  return quote&&Number.isFinite(quote.close)?{...quote,date:snapshot.date}:null;
};
...
function renderGlobal(){
  const g=market.global_context||{},rawItems=g.indicators||[],news=g.news||[];
  const indicatorOrder=[['^GSPC','S&P 500'],['^IXIC','那斯達克綜合指數'],['^DJI','道瓊工業指數'],['^SOX','費城半導體指數'],['^VIX','VIX恐慌指數']];
  const bySymbol=Object.fromEntries(rawItems.map(x=>[x.symbol,x]));
  const items=indicatorOrder.map(([symbol,name])=>({...bySymbol[symbol],symbol,name,missing:!bySymbol[symbol]}));
  ...
  $('globalIndicators').innerHTML=items.map(x=>'<div><span>'+escape(x.name)+'<small>'+escape(x.missing?'等待下次同步':x.stale?'前次資料':stateName[x.market_state]||'狀態未知')+(x.asof?' · '+escape(x.asof.replace('T',' ').slice(5)):'')+'</small></span><strong class="'+color(x.change_pct)+'">'+(Number.isFinite(x.change_pct)&&x.change_pct>=0?'+':'')+fmt(x.change_pct)+(Number.isFinite(x.change_pct)?'%':'')+'</strong></div>').join('');
...
function normalizeWatchPages(value){
  ...
  const pages=source.slice(0,10).map((page,i)=>({
    ...
    codes:[...new Set((Array.isArray(page?.codes)?page.codes:[]).map(String).map(x=>x.trim().toUpperCase()).filter(x=>/^\d{4,6}[A-Z]?$/.test(x)))].slice(0,10)
  }));
...
function moveArrayItem(items,from,to){
  ...
  const [item]=items.splice(from,1);items.splice(to,0,item);return true;
}
function renderWatchlist(){
  ...
  $('watchlist').innerHTML=(page.codes||[]).slice(0,10).map(code=>{
    const stock=current()[code]||latestMarket()[code],profile=market.company_profiles?.[code]||{},history=prices(code),fresh=latestClose(code),price=fresh?.close??last(history)?.close??stock?.quote?.close,previous=fresh?.previous_close??(history.length>1?history[history.length-2]?.close:null),net=current()[code]?.foreign?.net;
...
function renderWatchEditor(){
  $('editorPages').innerHTML=editingPages.map((page,i)=>{
    const chips=page.codes.map((code,j)=>'<span class="editorchip"><b>'+escape(code)+'</b><span>'+escape(current()[code]?.name||'查無上市資料')+'</span><span class="chipactions"><button type="button" data-move-code="-1" data-code-index="'+j+'" '+(j===0?'disabled':'')+' aria-label="上移 '+escape(code)+'">↑</button><button type="button" data-move-code="1" data-code-index="'+j+'" '+(j===page.codes.length-1?'disabled':'')+' aria-label="下移 '+escape(code)+'">↓</button><button type="button" class="removechip" data-remove-code="'+escape(code)+'" aria-label="刪除 '+escape(code)+'">×</button></span></span>').join('');
    return '<section class="editorpage" data-edit-page="'+i+'"><div class="editorpageorder"><strong>第 '+(i+1)+' 頁</strong><div><button type="button" data-move-page="-1" '+(i===0?'disabled':'')+' aria-label="分類上移">↑ 上移</button><button type="button" data-move-page="1" '+(i===editingPages.length-1?'disabled':'')+' aria-label="分類下移">↓ 下移</button></div></div><div class="editorpagehead"><label>大標題<input data-field="title" maxlength="24" value="'+escape(page.title)+'"></label><label>分類說明<input data-field="subtitle" maxlength="50" value="'+escape(page.subtitle)+'"></label><button type="button" class="deletepage" data-delete-page>刪除分類</button></div><div class="editorcodes">'+(chips||'<span class="editornote">尚未加入股票</span>')+'</div><form class="addstock"><input data-new-code inputmode="search" maxlength="7" autocapitalize="characters" placeholder="例如 2330、00981A"><button type="submit">＋ 新增個股</button></form></section>';
  }).join('');
}
...
function render(){
  ...
  const priceRows=prices(selected),a=analyze(priceRows,availableFlows),fresh=latestClose(selected),price=fresh?.close??last(priceRows)?.close??latestStock.quote?.close,pe=latestStock.pe;
  const previousClose=fresh?.previous_close??(priceRows.length>1?priceRows[priceRows.length-2]?.close:null);
  const closeClass=Number.isFinite(price)&&Number.isFinite(previousClose)?(price>previousClose?'pos':price<previousClose?'neg':'flat'):'';
  $('closePe').className='closepe';
  $('closePe').innerHTML=Number.isFinite(price)?'<span class="metricpart"><small>收盤價</small><b class="'+closeClass+'">'+fmt(price)+' 元</b></span><span class="metricdivider">｜</span><span class="metricpart"><small>本益比</small><b>'+(Number.isFinite(pe)&&pe>0?fmt(pe)+' 倍':'無資料')+'</b></span>':'<span class="metricpart"><small>收盤價</small><b>尚無資料</b></span>';
  $('closePeDate').textContent=fresh?.date?'當日收盤 · '+fresh.date:'最近盤後行情 · '+(last(priceRows)?.date||market.latest_session);
...
async function load(){
  ...
  try{const response=await fetch('data/market.json?cache='+Date.now(),{cache:'no-store'});if(!response.ok)throw new Error('HTTP '+response.status);market=await response.json();if(!market.latest_session||!latestMarket())throw new Error('尚未取得交易日資料。');institutionDate=market.latest_session;loadWatchPages();const recent=days().slice(-5).reverse();$('institutionDate').innerHTML=recent.map(date=>'<option value="'+escape(date)+'">'+escape(date)+(date===market.latest_session?'（最新）':'')+'</option>').join('');$('institutionDate').value=institutionDate;$('stamp').textContent='法人 '+market.latest_session+(market.latest_quotes?.date?' · 收盤 '+market.latest_quotes.date:'')+' · v'+(market.version||'2.4.13');const first=watchPagesData.flatMap(x=>x.codes||[]).find(x=>latestMarket()[x]);if(!latestMarket()[selected])selected=first||Object.keys(latestMarket())[0];render();}
...
$('search').addEventListener('change',e=>{const term=e.target.value.trim().toUpperCase(),list=Object.entries(current());const hit=list.find(([code])=>code===term)||list.find(([code,s])=>code.includes(term)||s.name.includes(term));if(hit){selected=hit[0];e.target.value='';render();}else if(term)$('status').textContent='沒有找到這個上市或上櫃股票；興櫃股票目前尚未收錄。';});
